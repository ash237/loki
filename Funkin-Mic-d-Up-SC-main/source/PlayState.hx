...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		*/
		...
		case 'co-op' | 'rumble' | 'all-nighter' | 'game-on':
		{
			defaultCamZoom = 0.7;
			curStage = 'room';
			switch (SONG.song.toLowerCase()) {
				case 'co-op' | 'rumble':
					var bg:FlxSprite = new FlxSprite(-1000, -400).loadGraphic(Paths.image('251', 'shared'));
					bg.scrollFactor.set(1, 1);
					bg.scale.set(0.65, 0.65);
					add(bg);
				case 'all-nighter' | 'game-on':
					var bg:FlxSprite = new FlxSprite(-1000, -400).loadGraphic(Paths.image('250', 'shared'));
					bg.scrollFactor.set(1, 1);
					bg.scale.set(0.65, 0.65);
					add(bg);
			}
		...
		switch (curStage)
		{
			...
			case 'room':
				...
				dad.x += 400;
				dad.y += 30;
				camPos.x = dad.getMidpoint().x + 150;
				camPos.y = dad.getMidpoint().y - 100;
		}

		add(gf);
		switch (SONG.song.toLowerCase()) {
			case 'Game-on' | 'All-nighter':
				gf.visible = false;
		}
		// Shitty layering but whatev it works LOL
		if (curStage == 'limo')
					...
					if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
						{
							...
							switch (curStage)
							{
								...
								case 'room':
									camFollow.x = FlxMath.lerp(camFollow.x, boyfriend.getMidpoint().x + 120, (camLerp * _variables.cameraSpeed)/(_variables.fps/60));
							}
					...
					if (SONG.validScore)
						{
							#if !switch
							trace(misses);
							Highscore.saveScore(SONG.song, songScore, storyDifficulty, misses==0);
							#end
						}
				}
					...
					case "Story":
						...
						if (storyPlaylist.length <= 0)
						{
							...
							if (SONG.validScore)
							{
								...
								Highscore.saveWeekScore(storyWeek, campaignScore, storyDifficulty, campaignMisses==0);
							}
...
private function popUpScore(strumtime:Float):Void
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'game-on' && curBeat == 188 && !dad.animation.curAnim.name.startsWith("jump")) {
			dad.playAnim('jump');
			FlxTween.tween(FlxG.camera, {zoom: 1.3}, (Conductor.stepCrochet * 8 / 1000), {ease: FlxEase.quadInOut});
			camFollow.x -= 250;
			camFollow.y -= 50;
			FlxTween.tween(camFollow, {x: camFollow.x - 400}, (Conductor.stepCrochet * 8 / 1000));
			FlxTween.tween(camFollow, {y: camFollow.y - 100}, (Conductor.stepCrochet * 8 / 1000));
		}
